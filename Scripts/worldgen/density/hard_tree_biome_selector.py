...
def generate_unified_selector_code_json_from_rows(
    ...
    def is_zero_expr(expr):
        ...
        if isinstance(expr, dict):
            ...
            if t == "minecraft:constant":
                return float(expr.get("argument", 1)) == 0.0
            if t in ("minecraft:cache_once", "minecraft:cache_2d", "minecraft:flat_cache"):
                return is_zero_expr(expr.get("argument"))
        return False
    def df_const(v):
        return {
    ...
    def clip_row_to_segment(row, dim_name, seg_lo, seg_hi, eps=1e-12):
        """
        Clip one row to the current segment on the active dimension.

        Important:
        This preserves parent-segment semantics, so child recursion only sees
        the restricted local subdomain instead of the original wider interval.
        """
        lo, hi = row_interval(row, dim_name)
    ...
    def compile_rows(rows, dim_idx=0):
        ...
        # ------------------------------------------------------------
        # IMPORTANT:
        # Do NOT do these unsafe collapses here:
        # - all_exprs_equal(child_exprs) -> return child_expr
        # - single full-domain branch -> return child_expr
        # These can incorrectly lift local-domain semantics upward.
        # ------------------------------------------------------------
    ...
    debug_info = {
        ...
        "mode": "unified_selector_biome_code_from_rows",
...
if __name__ == "__main__":
    ...
    selector_json, debug_info = generate_unified_selector_code_json_from_rows(
        biome_rows=filtered_biome_rows,
        biome_code_map=filtered_biome_code_map,
        output_path="biome_selector.json",
        print_debug=True
    )
    ...
    # print("[debug] unique biomes before compile =", sorted(set(row["biome"] for row in biome_rows))[:50])
    # print("[debug] unique biome count before compile =", len(set(row["biome"] for row in biome_rows)))
