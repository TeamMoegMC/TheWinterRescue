ServerEvents.recipes((event) => {
	...
	let recipes = [
		...
		{
			output:
				[Fluid.of("frostedheart:lime_water", 1000)],
			input:
		...
		{
			output:
				["8x immersiveengineering:treated_wood_horizontal"],
			input:
		...
		{
			output:
				["frostedheart:sodium_sulfide_dust"],
			input:
				["frostedheart:sodium_ingot", "#forge:dusts/sulfur"]
		},
		{
			output:
				["4x frostedheart:pulp"],
			input:
	...
	heated.forEach((recipe) => {
		create.mixing(recipe.output, recipe.input).heated()
	})
	create.filling("minecraft:leather", [
		"stone_age:raw_hide",
		Fluid.of("frostedheart:sulfur_dioxide", 250)
	])
})